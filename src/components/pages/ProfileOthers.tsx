...
function ProfileOthers() {
  ...
  useEffect(() => {
    ...
    err.push(<div></div>);
    err.push(
      <h3 style={{ textAlign: "center" }}>No projects available to show.</h3>
    );
    setProjectsList(err);
  }, [data]);
      ...
      for (let i = 0; i < data?.projectsCount; i++) {
        projectsList.push(
          <div
            ...
            onClick={() => {
              navigate(`/display-project/${projectData[i].projectId}`);
            }}
          >
    ...
    <div className="container centered profile-text">
      ...
      <div className="profile-container">
        ...
        <div className="about-container">
          ...
          <p>{data?.studies ? data?.studies : "Not given"}</p>
          <hr />
          <p className="bold allcaps">Occupation</p>
          <p>{data?.occupation ? data?.occupation : "Not given"}</p>
          <hr />
          <p className="bold allcaps">Work experience</p>
          <p>{data?.workExperience ? data?.workExperience : "Not given"}</p>
        </div>
        <hr />
        <p className="bold allcaps">A few words about me</p>
        <br />
        <p>{data?.aboutMe ? data?.aboutMe : "Not given"}</p>
        <hr />
